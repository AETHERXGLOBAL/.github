...
`AETHER X Live Portfolio Pulse` is a public-safe, automatically refreshed view of selected AETHER X portfolio-state signals plus a bounded organizational-state disclosure for the dedicated AETHER X Research unit.
...
- technical integration between AETHER X initiatives unless separately approved and evidenced;
...
The public card may state only approved organizational facts needed to understand the company structure. It must not expose the private research registry, unpublished research programs, scientific evidence, experimental status, IP analysis, publication-gate details or any claim not separately approved for public disclosure.

## Update model

The public SVG is generated at the existing compatibility path:
...
> **Naming note:** internal filenames and secret identifiers retain their existing `PROJECT_PULSE` names for automation compatibility. The public product name is **AETHER X Live Portfolio Pulse**.
